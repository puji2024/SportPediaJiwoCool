...
protocol FavoriteDataSource: Sendable {
...
final class FavoriteStore: @unchecked Sendable, FavoriteDataSource {
    ...
    private init() {
        ...
        container.loadPersistentStores { _, error in
            if let error { fatalError("Core Data gagal dimuat: \(error.localizedDescription)") }
        }
    ...
    private func publisher<Value>(_ operation: @escaping () throws -> Value) -> AnyPublisher<Value, Error> {
        Deferred {
            Future { promise in
                do { promise(.success(try operation())) }
    ...
    private func team(from object: NSManagedObject) -> Team? {
        guard let id = object.value(forKey: "id") as? String, let name = object.value(forKey: "name") as? String else { return nil }
        return Team(id: id, name: name, badgeURL: (object.value(forKey: "badgeURL") as? String).flatMap(URL.init(string:)), formedYear: object.value(forKey: "formedYear") as? String, stadium: object.value(forKey: "stadium") as? String, description: object.value(forKey: "teamDescription") as? String, country: object.value(forKey: "country") as? String)
    }
}
